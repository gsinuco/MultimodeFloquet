PROGRAM MULTIMODEFLOQUET
  ...
  FIELDS(2)%X         = 2.0*2.0*pi*0.389E6*hbar/(mu_B*0.5)
  FIELDS(2)%Y         = 0.0
  FIELDS(2)%Z         = 0.0
  FIELDS(2)%phi_x     = 0.0
  FIELDS(2)%phi_y     = 0.0
  FIELDS(2)%phi_z     = 0.0
  FIELDS(2)%omega     = 2.0*pi*2.2645E6
...
!!$!========= FIND THE MULTIMODE FLOQUET SPECTRUM 
     ...
     FIELDS(3)%omega = 2.0*pi*freq + 2*A/hbar
...
!!$        WRITE(*,*) (FIELDS(3)%OMEGA*A/HBAR)/(2*pi),t2,abs(U_AUX)**2
