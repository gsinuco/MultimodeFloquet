...
PROGRAM MULTIMODEFLOQUET
  ...
  INFO = 0
  D_BARE = 128
  CALL FLOQUETINIT('lattice','U',0.1D1*D_BARE,ID,INFO)  
  ...
  DO m=1,D_BARE-1
     FIELDS(1)%V(m,m+1)  = 0.125*EXP(DCMPLX(0.0, 1.0)*COS(2*PI*m/D_BARE))
     FIELDS(1)%V(m+1,m)  = 0.125*EXP(DCMPLX(0.0,-1.0)*COS(2*PI*m/D_BARE))
  END DO
