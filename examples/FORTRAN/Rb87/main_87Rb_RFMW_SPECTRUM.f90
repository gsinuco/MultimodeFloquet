PROGRAM QUDITSTABILITY
  ...
  DOUBLE PRECISION  :: B_DC,B_rf,omega_rf,B_mw,omega_mw
  INTEGER           :: INFO
  ...
  OPEN(UNIT=3,file="Rb87_RFMW_SPECTRUM_test.dat", action="write")
  ...
  OMEGA_MW  = 2*A/hbar!mu_B*abs(g_1)*B_DC/hbar
  
  !write(*,*) '#',omega_plus,omega_minus,g_2,g_1
  DO r=1,1!N_
     B_DC = 0.9E-4 + (r-1)*0.2E-4/N_     
     CALL RB87RFMWSPECTRUM(ID,B_DC,B_RF,omega_RF,B_MW,omega_MW,E_DRESSED,INFO)
...
!  TYPE(ATOM)                                       ID
  ...
  MANIFOLD_INDEX = FIELDS(3)%N_FLOQUET*(2*FIELDS(2)%N_FLOQUET+1)*3 + 1 + 3*(FIELDS(2)%N_FLOQUET)
  DO r = MANIFOLD_INDEX,MANIFOLD_INDEX+2!(2*FIELDS(2)%N_FLOQUET+1)*3
     !write(*,*) r,E_FLOQUET(r)
     E_QUDIT(r-manifold_index+1) = E_FLOQUET(r)
  END DO
  WRITE(*,*)
  MANIFOLD_INDEX = (2*FIELDS(3)%N_FLOQUET+1)*(2*FIELDS(2)%N_FLOQUET+1)*3 + 1 &
       & + FIELDS(3)%N_FLOQUET*(2*FIELDS(2)%N_FLOQUET+1)*5  + 5*(FIELDS(2)%N_FLOQUET)
  DO r = MANIFOLD_INDEX,MANIFOLD_INDEX+4!(2*FIELDS(2)%N_FLOQUET+1)*3
