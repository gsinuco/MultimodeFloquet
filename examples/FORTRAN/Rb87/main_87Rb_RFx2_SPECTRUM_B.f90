PROGRAM QUDITSTABILITY
  ...
  USE TYPES
  IMPLICIT NONE
  DOUBLE PRECISION, DIMENSION(8)   :: E_DRESSED,magneticmoment,E_QUDIT_RWA,magneticmoment_rwa
  DOUBLE PRECISION, DIMENSION(8,8) :: QUBITFREQUENCIES
  DOUBLE PRECISION, DIMENSION(15)  :: differentialmoments
  DOUBLE PRECISION  :: B_DC,B_plus,omega_plus,B_minus,omega_minus,B_ratio,RMS,gamma,B_DC_0
  INTEGER           :: INFO
...
!  OPEN(UNIT=4,file="Rb87_Qudit_ZeemanShift.dat", action="write")
  OPEN(UNIT=3,file="Rb87_RFx2_QuditSpectra_FloquetVsRWA.dat", action="write")
!  WRITE(3,*) "#1E4*B_DC, E_QUDIT_RWA,E_DRESSED"
  ...
  printingstring = "# plot ""Rb87_RFx2_QuditSpectra_FloquetFull4x3.dat""  u 1:2 w l"
...
!  write(*,*) trim(printingstring)
  ...
  DO l_ =1,N_
     ...
     B_PLUS      = 1.0*0.005E-5
     B_MINUS     = B_PLUS
     OMEGA_PLUS  = mu_B*abs(g_1)*B_DC_0/hbar
     
     B_DC        = B_DC_0 - 0.01*B_DC_0 + (l_-1)*0.02*B_DC_0/N_
     
     !write(*,*) '#',omega_plus,omega_minus,g_2,g_1
     DO r=1,1!N_
        gamma       = 1.0!(0.995 + (r-1)*0.01/N_)
        OMEGA_MINUS = gamma*mu_B*abs(g_1)*B_DC_0/hbar
        CALL RB87RFMWSPECTRUM(ID,B_DC,B_plus,omega_plus,B_minus,omega_minus,E_DRESSED,INFO)
        CALL RB87RFx2_LinearZeemanRWA(ID,B_DC,B_plus,omega_plus,B_minus,omega_minus,E_QUDIT_RWA,INFO)
        WRITE(3,*) 1E4*B_DC, E_DRESSED,E_QUDIT_RWA, mu_B*g_2*B_DC/A + 0.75 -hbar*omega_plus/A, & 
             & mu_B*g_1*B_DC/A - 1.25 + hbar*omega_minus/A
!        CALL RB87RFMW_MAGNETICMOMENT(ID,B_DC,B_plus,omega_plus,B_minus,omega_minus,MAGNETICMOMENT,INFO)
!        CALL RB87RFMW_MAGNETICMOMENT_RWA(ID,B_DC,B_plus,omega_plus,B_minus,omega_minus,MAGNETICMOMENT_RWA,INFO)
!        write(3,*) 1E4*B_DC, gamma,MAGNETICMOMENT_RWA,MAGNETICMOMENT
...
SUBROUTINE RB87RFMW_MAGNETICMOMENT_RWA(ID,B_DC,B_plus,omega_plus,B_minus,omega_minus,MAGNETICMOMENT,INFO)
...
END SUBROUTINE RB87RFMW_MAGNETICMOMENT_RWA
...
!  TYPE(ATOM)                                       ID
  ...
  ! ===================================================
!plot "Rb87_RFx2_QuditSpectra_FloquetVsRWA.dat" u 1:2 w l lw 5, "" u 1:10 w l lw 5, "Rb87_RFx2_QuditSpectra_FloquetFull3x1.dat" u 1:29, "" u 1:30 , "" u 1:31, "" u 1:32, "" u 1:33, "" u 1:34, "" u 1:35, "" u 1:36, "" u 1:37, "Rb87_RFx2_QuditSpectra_FloquetFull3x1_ZeroCoupling.dat" u 1:29 w l, "" u 1:30 w l, "" u 1:31 w l, "" u 1:32 w l, "" u 1:33 w l, "" u 1:34 w l, "" u 1:35 w l, "" u 1:36 w l, "" u 1:37 w l


  INTEGER :: t,o,OFFSET_C,OFFSET_R,MANIFOLD_INDEX,index_
  DOUBLE PRECISION :: E_DOWN,E_UP,g_1,g_2,gamma
  DOUBLE PRECISION :: freq,pop

  OPEN(UNIT=4,file="Rb87_RFx1_QuditSpectra.dat", access = "append",action="write")
  ...
  DO t=2,3
     DO r=1,3
        DO m= 4,8
!           FIELDS(t)%v(r,m) = DCMPLX(0.0,0.0)
!           FIELDS(t)%v(m,r) = DCMPLX(0.0,0.0)
        END DO
     END DO
!     call write_matrix(abs(fields(t)%V))
  END DO

!  FIELDS(2)%V(4:8,4:8) = 0.0
!  FIELDS(3)%V(1:3,1:3) = 0.0

!  DO r=1,8
!     E_QUDIT(r) = FIELDS(1)%V(r,r)
!  END DO
...
!  CALL WRITE_MATRIX(ABS(H_FLOQUET))
  ...
  gamma = omega_minus/(omega_plus*abs(g_1)/g_2)

  

  write(4,*) 1E4*B_DC,E_FLOQUET
...
!!$  DO r = MANIFOLD_INDEX-4,MANIFOLD_INDEX+4,4
!!$     index_ = (r-manifold_index+4)/4 +1 
!!$     E_QUDIT(index_) = E_FLOQUET(r)
!!$  END DO
!!$
!!$  MANIFOLD_INDEX = (2*FIELDS(3)%N_FLOQUET+1)*(2*FIELDS(2)%N_FLOQUET+1)*3  &
!!$       & + ((2*FIELDS(3)%N_FLOQUET+1)*(2*FIELDS(2)%N_FLOQUET+1)*5+1)/2 
!!$  DO r = MANIFOLD_INDEX-6,MANIFOLD_INDEX+6,3
...
SUBROUTINE RB87RFx2_LinearZeemanRWA(ID,B_DC,B_plus,omega_plus,B_minus,omega_minus,E_QUDIT,INFO)
  ...
  ! ===================================================
  !PARAMETERS REQUIRED TO DEFINE THE DRESSED BASIS
  COMPLEX*16,       DIMENSION(:,:), ALLOCATABLE :: U_FD,H_RWA     ! TRANSFORMATION FROM THE BARE TO THE DRESSED BASIS
  DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE   :: E_DRESSED      ! DRESSED SPECTRUM
  INTEGER                                       :: DRESSINGFIELDS, DRESSINGFLOQUETDIMENSION! NUMBER OF DRESSING FIELDS
  INTEGER,          DIMENSION(:), ALLOCATABLE   :: DRESSINGFIELDS_INDICES ! IDENTITY OF THE DRESSING FIELDS
...
!     call write_matrix(abs(fields(t)%V))
  ...
  !2. Build the Hamiltonian in the rotating frame

  DO r=1,3
     FIELDS(1)%V(r,r) = FIELDS(1)%V(r,r) - (r-2)*hbar*omega_minus/A
  END DO
  DO r=4,8
     FIELDS(1)%V(r,r) = FIELDS(1)%V(r,r) - (r-6)*hbar*omega_plus/A
  END DO
  
 !  call write_matrix(real(fields(1)%V))

  FIELDS(1)%V =FIELDS(1)%v + 0.5*FIELDS(2)%V + FIELDS(3)%V/2.0
  
 ! call write_matrix(real(fields(1)%V))

  !3. DIAGONALISE THE MULTIMODE FLOQUET
  ALLOCATE(E_FLOQUET(size(fields(1)%V,1)))
  ALLOCATE(H_RWA(size(fields(1)%V,1),SIZE(FIELDS(1)%V,1)))
  E_FLOQUET = 0.0  
  H_RWA = FIELDS(1)%V
  CALL LAPACK_FULLEIGENVALUES(H_RWA,SIZE(H_RWA,1),E_FLOQUET,INFO)
  E_QUDIT = E_FLOQUET
