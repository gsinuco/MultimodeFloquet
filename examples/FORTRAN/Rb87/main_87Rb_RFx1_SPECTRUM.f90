PROGRAM QUDITSTABILITY
  ...
  DOUBLE PRECISION  :: B_DC,B_RF,omega_RF,B_plus,B_minus
  INTEGER           :: INFO
  CHARACTER*7200 printingstring 
  character*20 charaux
  TYPE(ATOM) :: ID
  INTEGER r,N_,i_,j_,l_
  DOUBLE PRECISION :: g_1,g_2,RMS,B_ratio

  OPEN(UNIT=3,file="Rb87_RFx1_QuditSensitivityA.dat", action="write")
  ...
  DO l_ =1,N_
     ...
     B_ratio = 2.0 - l_*2.0/N_

     B_DC        = 0.5E-4
     B_plus      = 0.05E-4
     B_minus     = B_ratio*B_plus
     OMEGA_RF    = mu_B*g_2*B_DC/hbar
!---B
!    B_DC        = 3.2E-4
!    B_plus      = 0.2E-4
!    B_minus     = B_ratio*B_plus
...
!!$  printingstring = "plot "datos.dat" u 1 w l"
...
!!$  write(*,*) trim(printingstring)
     ...
     DO r=1,N_
        B_DC = 0.45E-4 + (r-1)*0.1E-4/N_!A
        !B_DC = 3.15E-4 + (r-1)*0.1E-4/N_!B
        CALL RB87RFMW_MAGNETICMOMENT(ID,B_DC,B_plus,B_minus,omega_RF,MAGNETICMOMENT,INFO)
        !CALL RB87RFMWSPECTRUM(ID,B_DC,B_plus,B_minus,omega_RF,E_DRESSED,INFO)
...
SUBROUTINE RB87RFMW_MAGNETICMOMENT(ID,B_DC,B_plus,B_minus,omega_RF,MAGNETICMOMENT,INFO)
  ...
  DOUBLE PRECISION,               INTENT(IN)    :: B_DC,B_plus,B_minus,omega_RF
  ...
  CALL RB87RFMWSPECTRUM(ID,B_DC,   B_plus,B_minus,omega_RF,E_QUDIT,   INFO)
  CALL RB87RFMWSPECTRUM(ID,B_DC+DB,B_plus,B_minus,omega_RF,E_QUDIT_DB,INFO)

  MagneticMoment = 1e-6*(A*(E_QUDIT_DB - E_QUDIT)/(2*pi*hbar))/(DB) ! In Hz/T
!  write(*,*) E_QUDIT_DB
!  WRITE(*,*) E_QUDIT
...
SUBROUTINE RB87RFMWSPECTRUM(ID,B_DC,B_plus,B_minus,omega_plus,E_QUDIT,INFO)
  ...
  DOUBLE PRECISION, INTENT(IN)    :: B_DC,B_plus,B_minus,omega_plus
  INTEGER,          INTENT(INOUT) :: INFO
...
!  TYPE(ATOM)                                       ID
  ...
  DOUBLE PRECISION                              :: T1,T2,B_RF_X,B_RF_Y
  ...
  INTEGER :: t,o,OFFSET_C,OFFSET_R,MANIFOLD_INDEX
...
!  MODES_NUM(3) = 1 !(DRIVING BY SIGMA MINUS)
  ...
  B_RF_X = B_plus + B_minus
  B_RF_Y = B_plus - B_minus
...
!  write(*,*) e_floquet
  ...
  MANIFOLD_INDEX =  1 + 3*(FIELDS(2)%N_FLOQUET)
  DO r = MANIFOLD_INDEX,MANIFOLD_INDEX+2!(2*FIELDS(2)%N_FLOQUET+1)*3
     !write(*,*) r,E_FLOQUET(r)
     E_QUDIT(r-manifold_index+1) = E_FLOQUET(r)
  END DO
  !WRITE(*,*)
  MANIFOLD_INDEX = (2*FIELDS(2)%N_FLOQUET+1)*3 + 1 + 5*(FIELDS(2)%N_FLOQUET)
  DO r = MANIFOLD_INDEX,MANIFOLD_INDEX+4
     E_QUDIT(r-manifold_index+4) = E_floquet(r)
     !write(*,*) r,E_FLOQUET(r)
  END DO
