...
!!$!========= FIND THE MULTIMODE FLOQUET SPECTRUM 
     ...
     FIELDS(3)%omega     = 2*A/hbar + FIELDS(2)%omega + mu_B*0.5*1.0*abs(FIELDS(2)%x)/hbar! + (r-1)*2.0*abs(FIELDS(2)%x)/128)/hbar
     
     ! ===== EVALUATE TIME-EVOLUTION OPERATOR 

     T1 = 0.0
     DO m=1,32
        T2 =(m-1)*2.0E-1/32!(2.0*pi/(0.125*FIELDS(2)%X/2.0))/1.0
