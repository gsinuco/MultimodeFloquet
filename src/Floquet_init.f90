...
SUBROUTINE FLOQUETINIT(atomicspecie,manifold,JTOTAL,ID,info)
  ...
  INFO = 4
  IF( PRESENT(ATOMICSPECIE) .AND. PRESENT(MANIFOLD) .AND. PRESENT(JTOTAL) .AND. PRESENT(ID)) THEN
     CALL SET_ATOMIC_PARAMETERS(ATOMICSPECIE,MANIFOLD,JTOTAL,ID,INFO)
  END IF
!  IF( PRESENT(ATOMICSPECIE) .AND. PRESENT(MANIFOLD) .AND. PRESENT(JTOTAL_HALF) .AND. PRESENT(ID)) THEN
!     CALL SET_ATOMIC_PARAMETERS(ATOMICSPECIE,MANIFOLD,0,JTOTAL,ID,INFO)
!  END IF
  ...
  IF(INFO.EQ.1 .AND. PRESENT(MANIFOLD).eqv. .TRUE.) THEN
