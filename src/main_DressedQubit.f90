PROGRAM MULTIMODEFLOQUET
  ...
  !parameters needed to define the dressed basis
  ...
  !=================================================================================
  !==== DEFINITION OF THE DRESSED BASIS
  !=================================================================================
  DRESSINGFIELDS = 2 
  ...
  CALL DRESSEDBASIS(ID,DRESSINGFLOQUETDIMENSION,DRESSINGFIELDS,SIZE(MODES_NUM,1),DRESSINGFIELDS_INDICES,MODES_NUM,FIELDS,&
       & U_FD,E_DRESSED,INFO)
  ...
  DO r=1,64
     FIELDS(3)%omega     = FIELDS(2)%X/4.0 + (r-1)*FIELDS(2)%X/(2.0*64)
...
!     write(*,*) r,E_floquet(size(E_floquet,1)/4:3*size(E_Floquet,1)/4),size(E_floquet,1)/4,3*size(E_floquet,1)/4
     ...
     !P_AVG = 0.0
     !WRITE(*,*) ID%D_BARE
     !CALL MULTIMODETRANSITIONAVG(SIZE(U_F,1),U_F,E_FLOQUET,ID%D_BARE,P_AVG,INFO) 
     ...
     DO m=1,128
        ...
        U_F1=0.0
        U_F2=0.0
        CALL MULTIMODEFLOQUETTRANSFORMATION(SIZE(U_FD,1),2,U_FD,E_DRESSED,ID%D_BARE,FIELDS(1:2),T1,U_F1,INFO) 
        CALL MULTIMODEFLOQUETTRANSFORMATION(SIZE(U_FD,1),2,U_FD,E_DRESSED,ID%D_BARE,FIELDS(1:2),T2,U_F2,INFO) 
